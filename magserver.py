#!/usr/bin/env python3
#
# Copyright (c) 2018-2022 Charles Godwin
#
...
from tzlocal import get_localzone  # noqa: F401 (imported but not used)

# Global dictionary of Magnum readers
magnumReaders = {}
...
class magServer(BaseHTTPRequestHandler):
    ...
    def _set_headers(self, contenttype="text/html"):
        ...
        # CORS headers (safe even when same-origin)
    ...
    def serve_index(self):
        ...
        try:
            with open(index_path, "rb") as f:
                content = f.read()
            self._set_headers(contenttype="text/html; charset=utf-8")
            self.wfile.write(content)
        except FileNotFoundError:
            self.send_error(HTTPStatus.NOT_FOUND, "index.html not found")

    def do_GET(self):
        # Route "/" and "/index.html" to the HTML UI
        if self.path == "/" or self.path == "/index.html":
            return self.serve_index()

        # JSON API now lives at /api
        if self.path == "/api":
            response = []
            code = None
            message = None
            ...
            for comm_device, magnumReader in magnumReaders.items():
                try:
                    ...
                    message = (
                        "Exception detected attempting to read network data - "
                        "{0} {1}".format(comm_device, str(e))
                    )
            ...
            return self.send_error(code, message=message)  # type: ignore
...
parser = MagnumArgumentParser(
    description="Magnum Network Reader",
